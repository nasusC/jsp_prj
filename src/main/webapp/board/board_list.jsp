<%@ page import="kr.co.sist.user.board.BoardDAO" %>
<%@ page import="java.sql.SQLException" %>
<%@ page import="kr.co.sist.user.board.SearchVO" %>
<%@ page import="kr.co.sist.user.board.BoardVO" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.ArrayList" %>
<%@ page import="kr.co.sist.util.BoardUtil" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" info="" %>
<!DOCTYPE html>
<html lang="ko" xmlns:th="http://www.thymeleaf.org">
<head>
  <meta charset="UTF-8">
  <title>board_list</title>
  <link rel="stylesheet" href="../common/css/main_20240911.css">
  <!--    bootstrap CDN  start-->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"
          integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz"
          crossorigin="anonymous"></script>
  <!--    JQuery CDN start-->
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js"></script>

  <style>
      a {
          color: black;
          text-decoration: none;
          font-size: 17px;
      }

      a:hover {
          color: #858585;
          text-decoration: underline;
          font-size: 22px;
      }
  </style>
</head>
<body>
<div id="wrap">
  <div id="header">
    <c:import url="../common/header.jsp"/>
  </div>
  <div style="width: 1010px">
    <h2 onclick="location.href='board_list.jsp'">사용자 자유 게시판</h2>
    <jsp:useBean id="sVO" class="kr.co.sist.user.board.SearchVO" scope="page"/>
    <jsp:setProperty name="sVO" property="*"/>
    <%
      // 게시판 리스트 구현

      // 1. 총 레코드 수 구하기
      int totalCount = 0;
      BoardDAO bDAO = BoardDAO.getInstance();

      try {
        totalCount = bDAO.selectTotalCount(sVO);

      } catch (SQLException se) {
        se.printStackTrace();
      }

      // 2. 한 화면에 보여줄 레코드의 수
      int pageScale = 10;

      // 3. 총 페이지 수
//    int totalPage = (int)Math.ceil(((double)totalCount/pageScale));
      int totalPage = 0;
      int remain = totalCount % pageScale;
      if (remain != 0) {
        totalPage = totalCount / pageScale + 1;
      } else {
        totalPage = totalCount / pageScale;
      }


      // 4. 검색의 시작번호를 구하기 (pagination의 번호) [1],[2],[3]
      String paramPage = request.getParameter("currentPage");

      int currentPage = 1;
      if (paramPage != null) {
        try {
          currentPage = Integer.parseInt(paramPage);
        } catch (NumberFormatException nfe) {

        }
      }
      // 시작번호
      // [1,10] , [11,20] ,,, 구하는 식
      int startNum = currentPage * pageScale - pageScale + 1;
      int endNum = startNum + pageScale - 1;

      sVO.setStartNum(startNum);
      sVO.setEndNum(endNum);
      sVO.setTotalPage(totalPage);
      sVO.setTotalCount(totalCount);
      sVO.setCurrentPage(currentPage);

      List<BoardVO> listBoard = null;
      try {
        listBoard = bDAO.selectBoard(sVO); // 시작번호, 끝 번호를 사용한 게시글 조회
        String tempSubject = null;
        for (BoardVO tempVO : listBoard) {
          tempSubject = tempVO.getSubject();
          if (tempSubject.length() > 30) {
            tempVO.setSubject(tempSubject.substring(0, 29) + "....");
          }
        }
      } catch (SQLException se) {
        se.printStackTrace();
      }


      pageContext.setAttribute("totalCount", totalCount);
      pageContext.setAttribute("pageScale", pageScale);
      pageContext.setAttribute("totalPage", totalPage);
      pageContext.setAttribute("currentPage", currentPage);
//      pageContext.setAttribute("startNum", startNum);
//      pageContext.setAttribute("endNum", endNum);
      pageContext.setAttribute("listBoard", listBoard);

    %>
    <%--    총 게시물의 수 : <c:out value="${totalCount}"/><br>--%>
    <%--    한 화면에 보여줄 게시물의 수 : <c:out value="${pageScale}"/><br>--%>
    <%--    총 페이지 수 : <c:out value="${totalPage}"></c:out><br>--%>
    <%--    현재 페이지 번호 : <c:out value="${currentPage}"/><br>--%>
    <%--    게시글의 시작 번호 : <c:out value="${startNum}"/><br>--%>
    <%--    게시글의 끝 번호 : <c:out value="${endNum}"/><br>--%>


    <div id="boardList" style="height: 500px;">
      <c:set var="loginFlag" value="javascript:loginMove()"/>
      <c:if test="${not empty userData}">
        <c:set var="loginFlag" value="board_write_frm.jsp"/>
      </c:if>

      <table class="table table-hover">
        <thead>
        <tr>
          <th style="width: 60px">번호</th>
          <th style="width: 550px">제목</th>
          <th style="width: 120px">작성자/ip</th>
          <th style="width: 200px">작성일</th>
        </tr>
        </thead>
        <tbody>
        <c:if test="${empty listBoard}">
          <tr>
            <td style="text-align: center" colspan="4">작성된 게시글이 없습니다.<br>
              <a href="board_write_frm.jsp">글쓰기</a>
            </td>
          </tr>
        </c:if>
        <c:if test="${not empty param.keyword}">
          <c:set var="searchParam" value="&field=${param.field}&keyword=${param.keyword}"/>
        </c:if>

        <c:forEach var="bVO" items="${listBoard}" varStatus="i">
          <tr>
            <td><c:out value="${totalCount-((currentPage-1)*pageScale)-i.index}"/></td>
            <td><a href="board_detail2.jsp?num=${bVO.num}&currentPage=${currentPage}${searchParam}">
              <c:out
                  value="${bVO.subject}"/></a></td>
            <td><c:out value="${bVO.writer}/${bVO.ip}"/></td>
            <td><fmt:formatDate value="${bVO.input_date}" pattern="yyyy-MM-dd EEEE HH:mm"/></td>
          </tr>
        </c:forEach>
        </tbody>
      </table>
      <button style="float: right" class="btn btn-success" onclick="location.href='${loginFlag}'">글쓰기</button>
    </div>
    <div id="search" style="height: 60px; text-align: center">
      <form action="board_list.jsp" method="GET" name="searchFrm" id="searchFrm">
        <select name="field" id="field">
          <%--          만약 제목을 선택했으면 url에 쿼리스트링(field=0)일것인데 3항 연산자를 통해 selected를 걸어줌으로써 0유지.--%>
          <option value="0" ${param.field eq '0' ? 'selected' : ''}>제목</option>
          <option value="1" ${param.field eq '1' ? 'selected' : ''}>내용</option>
          <option value="2" ${param.field eq '2' ? 'selected' : ''}>작성자</option>
        </select>
        <%--        입력된 값을 쿼리스트링(keyword)에서 가져와서 value에 저장함--%>
        <input type="text" id="keyword" name="keyword" style="width: 200px" value="${param.keyword}">
        <button id="searchBtn" class="btn btn-primary">검색</button>
        <input type="text" style="display: none">
      </form>
    </div>
    <div id="pagination" class="pagination d-flex justify-content-center">
      <%
        sVO.setUrl("board_list.jsp");
      %>
      <%= new BoardUtil().pagination(sVO)%>

    </div>
  </div>
</div>
<script>
    $(function () {
        $('#searchBtn').click(() => {
            chkNull();
        })
        $('#keyword').keyup((e) => {
            if (e.which == 13) {
                chkNull();
            }
        })

        // 검색으로 선택한 컬럼명과 키워드를 설정 (JSP코드로 작성가능)
        if (${not empty param.keyword}) {
            $('#field').val(${param.field});
            $('#keyword').val(${param.keyword});
        }

    })

    function loginMove() {
        if (confirm('로그인 한 사용자만 글쓰기 가능합니다.\n로그인 하시겠습니까?')) {
            location.href = '../login/login_frm.jsp';
        }
    }

    function chkNull() {
        var keyword = $('#keyword').val();
        if (keyword.length < 2) {
            alert('검색 키워드는 1글자 이상입니다.');
            $('#keyword').focus();
            return;
        }
        $('#searchFrm').submit();
    }
</script>
</body>
</html>